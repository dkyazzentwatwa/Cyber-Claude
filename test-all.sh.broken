...
# Check for timeout command (Linux has it, macOS doesn't by default)
...
# Function to run command with optional timeout and verbose output
run_test() {
    local cmd=$1
    local timeout_seconds=${2:-0}

    # Add timeout if available and requested
    if [ -n "$TIMEOUT_CMD" ] && [ $timeout_seconds -gt 0 ]; then
        cmd="$TIMEOUT_CMD ${timeout_seconds}s $cmd"
    fi
...
run_test "$CLI webscan https://example.com --quick > /dev/null 2>&1
...
run_test "$CLI cve CVE-2024-21762 > /dev/null 2>&1
test_result $? "Direct CVE lookup"

# Test 4.2: Keyword search
run_test "$CLI cve apache > /dev/null 2>&1
...
run_test "$CLI logs "$TEST_LOG" > /dev/null 2>&1
...
run_test "$CLI daemon status > /dev/null 2>&1
test_result $? "Daemon status check"

# Test 6.2: List jobs
run_test "$CLI daemon jobs > /dev/null 2>&1
test_result $? "Daemon jobs list"

# Test 6.3: Add job
JOB_OUTPUT=$(run_test "$CLI daemon add -n "Test Job" -t webscan -T https://example.com -s "0 0 * * *" 2>&1)
if [ $? -eq 0 ]; then
    ...
    # Test 6.4: Remove job
    if [ -n "$JOB_ID" ]; then
        run_test "$CLI daemon remove "$JOB_ID" > /dev/null 2>&1
...
if [ -z "$ANTHROPIC_API_KEY" ] && [ -z "$GOOGLE_API_KEY" ]; then
    ...
    run_test "$CLI auto "check system version" > /dev/null 2>&1
    test_result $? "Simple autonomous task"

    # Test 7.2: Complex autonomous task
    run_test "$CLI auto "quick security check" > /dev/null 2>&1
...
run_test "$CLI recon example.com --quick > /dev/null 2>&1
...
run_test "$CLI pcap "$TEST_PCAP" > /dev/null 2>&1
if [ $? -eq 0 ] || [ $? -eq 1 ]; then
...
echo "help" | run_test "$CLI > /dev/null 2>&1
test_result $? "Interactive session help"

# Test 10.2: Interactive status
echo "status" | run_test "$CLI > /dev/null 2>&1
