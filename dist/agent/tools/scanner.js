...
export class DesktopScanner {
    ...
    async scanSystem() {
        try {
            ...
            const [osInfo, system, processes, services, networkConnections, fsSize, users,] = await Promise.all([
                si.osInfo(),
                si.system(),
                si.processes(),
                si.services('*'),
    ...
    async quickCheck() {
        try {
            ...
            const uncommonPorts = listeningPorts.filter(p => p && typeof p === 'number' && p > 1024 && p < 49152 && ![3000, 5000, 8000, 8080, 9000].includes(p));
