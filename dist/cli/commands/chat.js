...
export function createChatCommand() {
    ...
    command
        ...
        ui.box(`You are now chatting with Cyber Claude in ${chalk.bold(options.mode)} mode.\n\n` +
            `Commands:\n` +
            `  ${chalk.cyan('/mode <mode>')} - Switch agent mode\n` +
            `  ${chalk.cyan('/clear')} - Clear conversation history\n` +
            `  ${chalk.cyan('/help')} - Show help\n` +
            `  ${chalk.cyan('/exit')} - Exit chat mode\n\n` +
            `Type your security questions or requests below.`, '💬 Chat Mode', 'info');
...
async function handleChatCommand(command, agent) {
